# /Users/robinsongarcia/projects/AzimutalEquidistant/projection/AzimutalEquidistant/config.py
...
logger = logging.getLogger('AzimutalEquidistant_projection.AzimutalEquidistant.config')

class AzimutalEquidistantConfigModel(BaseModel):
    """
    Pydantic model for AzimutalEquidistant projection configuration.
...
class AzimutalEquidistantConfig:
    """
    Configuration class for AzimutalEquidistant projections using Pydantic for validation.

    This class encapsulates all necessary parameters required to perform both forward (equirectangular to AzimutalEquidistant)
    and inverse (AzimutalEquidistant to equirectangular) projections. It ensures that configurations are validated
    and managed efficiently.
    """

    def __init__(self, **kwargs: Any) -> None:
        """
        Initialize the AzimutalEquidistantConfig with provided parameters.
        ...
        """
        logger.debug("Initializing AzimutalEquidistantConfig with parameters: %s", kwargs)
        try:
            self.config = AzimutalEquidistantConfigModel(**kwargs)
            logger.info("AzimutalEquidistantConfig initialized successfully.")
        except Exception as e:
            error_msg = f"Failed to initialize AzimutalEquidistantConfig: {e}"
    ...
    def update(self, **kwargs: Any) -> None:
        ...
        """
        logger.debug(f"Updating AzimutalEquidistantConfig with parameters: {kwargs}")
        try:
            updated_config = self.config.copy(update=kwargs)
            self.config = updated_config
            logger.info("AzimutalEquidistantConfig updated successfully.")
        except Exception as e:
            error_msg = f"Failed to update AzimutalEquidistantConfig: {e}"
            logger.exception(error_msg)
            raise ConfigurationError(error_msg) from e
    ...
    def __getattr__(self, item: str) -> Any:
        ...
        """
        logger.debug(f"Accessing AzimutalEquidistantConfig attribute '{item}'.")
        try:
            return getattr(self.config, item)
        except AttributeError:
            error_msg = f"'AzimutalEquidistantConfig' object has no attribute '{item}'"
    ...
    def __repr__(self) -> str:
        ...
        """
        return f"AzimutalEquidistantConfig({self.config.dict()})"