...
class ObliqueMercatorTransformer(BaseCoordinateTransformer):
    ...
    def spherical_to_image_coords(self, lat: np.ndarray, lon: np.ndarray, shape: tuple):
        ...
        H, W = shape[:2]
        print("DEBUG: lat range",lat.min(),lat.max())
        print("DEBUG: lon range",lon.min(),lon.max())
        ...
        # Example debugging snippet in transform code:
        print("DEBUG: map_x range:", map_x.min(), map_x.max())
        print("DEBUG: map_y range:", map_y.min(), map_y.max())
        return map_x, map_y
